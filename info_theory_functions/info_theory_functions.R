...
#=============================================================================
# get_ee1D
# This is a wrapper to cycle through and get the Excess Entropy
# for each pair site on a 1D lattice. 
#
# pop_ts			Population matrix with time as rows, each species as column
# k 				The size of the block history to be used. 
#=============================================================================
get_ee1D = function (pop_ts, k=2, s=1 ) {
	ngens = dim(pop_ts)[1] #Time
	nspp = dim(pop_ts)[2] #Number of species
	
	ee_web = matrix(0,ngens,nspp)

	for ( n in 1:nspp) {
		ee_temp = get_ais( pop_ts,k=k,s=s,focal=n)
		ee_web[k:(ngens-1),n] = ee_temp$local
...
#=============================================================================
# get_ais1D
# This is a wrapper to cycle through and get the Active Information Storage
# for each pair site on a 1D lattice. 
#
# pop_ts			Population matrix with time as rows, each species as column
# k 				The size of the block history to be used. 
#=============================================================================
get_ais1D = function (pop_ts, k=2, s=1 ) {
	ngens = dim(pop_ts)[1] #Time
	nspp = dim(pop_ts)[2] #Number of species
	
	ais_web = matrix(0,ngens,nspp)

	for ( n in 1:nspp) {
		ais_temp = get_ais( pop_ts,k=k,s=s,focal=n)
		ais_web[k:(ngens-1),n] = ais_temp$local
...
#get_te_left
# This is a wrapper to cycle through and get the Transfer Entropy on a pairwise
# basis for each pair of sites in a 1D lattice, from left to right. 
#
# pop_ts			Population matrix with time as rows, each species as column
# k 				The size of the block history to be used. 
#=============================================================================
get_te1D_left = function (pop_ts, k=2, s=1 ) { 

	ngens = dim(pop_ts)[1] #Time
	nspp = dim(pop_ts)[2] #Number of species
	
	te_web = matrix(0,ngens,nspp)

	for ( n in 1:nspp) {
		if ( n == nspp) { n2 = 1} else { n2 = n+1 }
		#print( paste( n," ",n2) )
		te_temp = get_TE( pop_ts[,c(n,n2)],k=k,s=s,focal=1)
		te_web[k:(ngens-1),n] = te_temp$local
		#In this format, each row is the effect of n2 ON n. Might be 
		#more logical to transpose this matrix so that each row is the
		#information from n TO n2. 
...
#get_te_right
# This is a wrapper to cycle through and get the Transfer Entropy on a pairwise
# basis for each pair of sites in a 1D lattice, from right to left. 
#
# pop_ts			Population matrix with time as rows, each species as column
# k 				The size of the block history to be used. 
#=============================================================================
get_te1D_right = function (pop_ts, k=2, s=1 ) { 
...
#get_si1D_left
# This is a wrapper to cycle through and get the Separable Information on a 
# pairwise basis on a 1D lattice, for pairs of sites from left to right. 
#
# pop_ts			Population matrix with time as rows, each species as column
# k 				The size of the block history to be used. 
#=============================================================================
get_si1D_left = function (pop_ts, k=2, s=1 ) { 

	ngens = dim(pop_ts)[1] #Time
	nspp = dim(pop_ts)[2] #Number of species
	
	si_web = matrix(0,ngens,nspp)
	...
	for ( n in 1:nspp) {
		...
		#print( paste( n," ",n2) )
		si_temp = get_SI( pop_ts[,c(n,n2)],k=k,s=s,focal=1)
		si_web[k:(ngens-1),n] =si_temp$local
	}
	return((si_web))
}
...
#get_si1D_right
# This is a wrapper to cycle through and get the Separable Information on a 
# pairwise basis on a 1D lattice, for pairs of sites from right to left.
#
# pop_ts			Population matrix with time as rows, each species as column
# k 				The size of the block history to be used. 
#=============================================================================
get_si1D_right = function (pop_ts, k=2, s=1 ) { 
